    ...
    for (const dayAssignments of Object.values(dailyAssignments)) {
        for (const rideId of Object.keys(dayAssignments)) {
            ...
            if (operatorId === currentUser.id) {
                ...
                if (rideName) {
                    operatedRides.add(rideName as string);
                }
            }
        }
    }

    return Array.from(operatedRides).sort();
            ...
            {operatorExpertise.length > 0 && (
                 <div className="mt-12">
                    ...
                    <div className="bg-gray-800 rounded-lg shadow-lg border border-gray-700 p-4">
                        ...
                        <ul className="columns-1 sm:columns-2 gap-x-6 list-disc list-inside text-gray-300">
                            {operatorExpertise.map(rideName => (
                            <li key={rideName} className="mb-1">{rideName}</li>
                            ))}
                        </ul>
                    </div>
                </div>
        ...
        {isRosterEmpty ? (
            <div className="text-center py-16">
                ...
                <p className="text-gray-500 mt-2">
                    {role === 'operator' 
                        ? "Your assignment information will appear here." 
                        : "The operator list is managed by the development team. Please contact them for any changes."
                    }
                </p>
            </div>
        ) : (
            <>
                        ...
                        <div key={operator.id} className="bg-gray-800 rounded-lg shadow-lg border border-gray-700 p-4 flex flex-col">
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <div className="flex items-center gap-2 text-sm">
                                    <span className={`w-3 h-3 rounded-full ${operator.attendance ? 'bg-green-400 animate-pulse' : 'bg-red-500'}`}></span>
                                    <span className={operator.attendance ? 'text-green-400' : 'text-red-500'}>
                                        {operator.attendance ? 'Checked In' : 'Not Checked In'}
                                    </span>
                                </div>
                            </div>
                                ...
                                <ul className="space-y-2 text-sm">
                                    {operatorAssignments && operatorAssignments.length > 0 ? (
                                        operatorAssignments.map(ride => (
                                            <li key={ride.id} className="text-gray-300 bg-gray-700/50 p-2 rounded-md">{ride.name} <span className="text-xs text-gray-500">({ride.floor} Floor)</span></li>
                                        ))
                                    ) : (
                ...
                {isManager && unassignedRides.length > 0 && (
                <div className="mt-12">
                    <h2 className="text-2xl font-bold text-pink-500 mb-4">Unassigned Rides</h2>
                    <div className="bg-gray-800 rounded-lg shadow-lg border border-gray-700 p-4">
