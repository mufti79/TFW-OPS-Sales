...
const DailySalesEntry: React.FC<DailySalesEntryProps> = ({ 
    ...
    }, [xtremeQty, kiddoQty, vipQty, otherSales, originalRecord]);
    ...
    const totalAmount = useMemo(() => xtremeAmount + kiddoAmount + vipAmount + otherAmountTotal, [xtremeAmount, kiddoAmount, vipAmount, otherAmountTotal]);
        ...
        for (const date in packageSales) {
            if (date >= mySalesStartDate && date <= mySalesEndDate) {
                ...
                if (userRecord) {
                    ...
                    summary.totalAmount += (userRecord.xtremeAmount || 0) + (userRecord.kiddoAmount || 0) + (userRecord.vipAmount || 0) + otherTotal;
                }
            }
        }
        return summary;
            ...
            <div className="mt-8 bg-gray-800/50 rounded-lg border border-gray-700 p-6 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="text-center sm:text-left">
                 ...
                 <div className="text-center sm:text-right">
                    ...
                    <p className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-teal-500">{totalAmount.toLocaleString()} BDT</p>
                </div>
            </div>
                ...
                <div className="bg-gray-800 rounded-lg shadow-lg border border-gray-700 p-6 space-y-4">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center pt-4 border-t-2 border-teal-500 gap-4">
                        ...
                        <div className="text-center sm:text-right">
                            ...
                            <p className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-teal-500">{salesSummary.totalAmount.toLocaleString()} BDT</p>
                        </div>
                    </div>
                </div>
            </div>
