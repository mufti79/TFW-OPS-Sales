...
interface OperatorManagerProps {
  operators: Operator[];
  onClose: () => void;
  onSave: (operators: Operator[]) => void;
  onImport: (newOperators: Operator[], strategy: 'merge' | 'replace') => void;
}

const OperatorManager: React.FC<OperatorManagerProps> = ({ operators, onClose, onSave, onImport }) => {
  ...
  useEffect(() => {
    ...
    setInternalOperators(JSON.parse(JSON.stringify(operators)));
  }, [operators]);

  const handleSaveChanges = () => {
    onSave(internalOperators);
    setShowSuccessMessage(true);
  };

  const handleImportAndShowSuccess = (strategy: 'merge' | 'replace') => {
    if (pendingImport) {
  ...
  const handleAddOperator = () => {
    if (newOperatorName.trim() && !internalOperators.some(op => op.name.toLowerCase() === newOperatorName.trim().toLowerCase())) {
      const maxId = internalOperators.reduce((max, op) => Math.max(op.id, max), 0);
      setInternalOperators(prev => [...prev, { id: maxId + 1, name: newOperatorName.trim() }]);
      setNewOperatorName('');
  ...
  const handleDeleteOperator = (id: number) => {
    if (window.confirm('Are you sure you want to delete this operator? This action will be saved when you click "Save Changes".')) {
        setInternalOperators(prev => prev.filter(op => op.id !== id));
    }
  };
  ...
  const handleToggleSelectAll = () => {
    if (selectedIds.size === internalOperators.length) {
      setSelectedIds(new Set());
    } else {
      setSelectedIds(new Set(internalOperators.map(op => op.id)));
    }
  };
  
  const handleDeleteSelected = () => {
    if (selectedIds.size === 0) return;
    if (window.confirm(`Are you sure you want to delete ${selectedIds.size} selected operator(s)? This action will be saved when you click "Save Changes".`)) {
      setInternalOperators(prev => prev.filter(op => !selectedIds.has(op.id)));
      setSelectedIds(new Set());
    }
  };

  const handleDeleteAll = () => {
    if (window.confirm('WARNING: This will delete ALL operators from the list. This action will be saved when you click "Save Changes" and cannot be undone.\n\nAre you sure you want to proceed?')) {
    ...
    reader.onload = (e) => {
        try {
            ...
            for(let i = 1; i < json.length; i++) {
                ...
                if (name) {
                    importedOperators.push({ id: 0, name });
                }
            }
            ...
            if (importedOperators.length > 0) {
                ...
                alert('No valid operator names found in the first column of the file (after the header row).');
            }
        } catch (error) {
            console.error("Error parsing Excel file:", error);
            alert("Failed to parse the file. Please ensure it's a valid Excel (xlsx) or CSV file with names in the first column.");
        } finally {
  ...
  const handleExportOperators = () => {
    if (operators.length === 0) {
      alert("There are no operators to export.");
      return;
    }

    const exportData = operators.map(op => ({ "Operator Name": op.name }));
  ...
  const isAllSelected = internalOperators.length > 0 && selectedIds.size === internalOperators.length;

  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" role="dialog" aria-modal="true">
      <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-lg border border-gray-700 animate-fade-in-up flex flex-col relative">
        {showSuccessMessage ? (
            <div className="p-8 text-center flex flex-col">
                <div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-green-500 mb-4 flex-shrink-0">
            ...
            <>
                <div className="p-6">
                    ...
                    <div className="space-y-2 max-h-[calc(50vh-80px)] overflow-y-auto pr-2">
                        {internalOperators.sort((a,b) => a.name.localeCompare(b.name)).map(op => (
                        ...
                        {internalOperators.length === 0 && <p className="text-gray-400 text-center py-4">No operators have been added yet.</p>}
                    </div>
                ...
                <div className="bg-gray-700/50 px-6 py-4 flex justify-end gap-4 rounded-b-lg mt-auto">
                    ...
                    <button onClick={handleSaveChanges} className="px-6 py-2 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 active:scale-95 transition-all">
                    Save Changes
                    </button>
                </div>
            </>
            ...
            <div className="absolute inset-0 bg-black bg-opacity-70 flex items-center justify-center z-10 p-4 rounded-lg">
                <div className="bg-gray-700 p-8 rounded-lg shadow-xl border border-gray-600 animate-fade-in-up text-center w-full max-w-sm">
                    ...
                    <div className="space-y-4">
                        <button 
                          onClick={() => handleImportAndShowSuccess('merge')}
                        ...
                        <button 
                          onClick={() => { 
                              if (window.confirm("WARNING:\n\nThis will DELETE ALL existing operators and their entire assignment and attendance history.\n\nThis action cannot be undone. Are you sure?")) {
                                  handleImportAndShowSuccess('replace');
                              }
                          }}
